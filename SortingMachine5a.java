...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        if (left < last && right < last) {
            //make sure they are examinable
            leftR = array.entry(left);
            rightR = array.entry(right);

            if (order.compare(leftR, rightR) < 0) {
                //if left is less than right
                if (order.compare(root, rightR) <= 0) {
                    // if root < right
                    array.exchangeEntries(top, right);
                    siftDown(array, right, last, order);

                } else {
                    if (order.compare(root, leftR) <= 0) {
                        //if left < root
                        array.exchangeEntries(top, right);
    ...
    private static <T> void heapify(Array<T> array, int top,
        ...
        if (isHeap(array, top, last, order)) {
            ...
            //array is not a heap, heapify both sides
            if (left < last) {
    ...
    private static <T> Array<T> buildHeap(Queue<T> q, Comparator<T> order) {
         ...
         */
        Array<T> returnArray = new Array1L<>(q.length());
        for (int pos = 0; q.length() > 0; pos++) {
            returnArray.setEntry(pos, q.dequeue());
        }
        heapify(returnArray, 0, order);
        return returnArray;
        // TODO - fill in body
    }
    ...
    public final T removeFirst() {
        ...
        T root = this.heap.entry(0); // return val
    ...
    public final int size() {
        ...
        int returnVal;
        if (this.insertionMode) {
            returnVal = this.entries.length();
        } else {
            returnVal = this.heapSize;
        }
